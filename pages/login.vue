			...
			<view class="boxcenter main_content">
					...
					<view class="boxcenter loginarea">
						<view  class="inputitem boxcenter inputtext" style="align-content: center;">
							...
							<view class="">
									<input type="number" value="18695732895" maxlength="11" @input="inputusername" placeholder="输入登录用户名" />
							</view>
						...
						<view  class="inputitem boxcenter inputtext">
							...
							<view class="">
								<input type="text" password="true" maxlength="11" value="18695732895" @input="inputpassword" placeholder="输入登录密码" />
							</view>
...
<script>
	...
	export default {
		...
		methods: {
			...mapMutations(['login']),
			inputpassword(e){
				this.logininfo.user_pass=e.target.value;
			},
			inputusername(e){
				this.logininfo.phone=e.target.value;
			},
	 ...
	 //方式二  then/catch
        sendRequest2 (e) {
		var that=this;
		if(that.logininfo.phone==""||that.logininfo.user_pass==""){
			uni.showModal({
				title:'你是不是忘了什么',
				showCancel:false
			})
		...
		this.$apiconfig.login_f({data:that.logininfo}).then(res=>{
			...
			if(res.data.code=="200"){
						...
						that.$request.setConfig({
						  ...
						  ///获取用户相关的组群，好友，及个人信息
						  let tempuid=res.data.data.id;
						   that.$apiconfig.initInfo_f({data:{usign:tempuid}}).then(res=>{
							 // if(res.data.data.group.length==0){
								//  res.data.data.group=new Object();
							 // }
							 // if(res.data.data.friend.length==0){
							 // 	 res.data.data.friend=new Object();
							 // }
					     ...
					     that.$mysocket.connectserver(tempuid,res.data.data.token);
				}else{
						uni.showModal({

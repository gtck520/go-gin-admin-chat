	...
	export default {
		...
		onLoad(option) {
		   ...
		   if(that.$mysocket.listenlist[this.chattype+this.chatto]){
			   ...
			   that.$mysocket.sockettast.onMessage(function(res){
				...
				/*  uni.getStorage({
					...
					complete(res) {
					   ...
					   if(res.data==''||typeof(res.data)=='undefined'){
						  ...
						  if(typeof(res.data[tempmy.id])=='undefined'||typeof(res.data[tempmy.id]['chatlog'])=='undefined'){
								console.log(1)
							}else{
					        
								if(that.chattype=='group'){
									if(typeof(res.data[tempmy.id]['chatlog']['group'+that.chatto])=='undefined'){
									
									}else{
									   list= res.data[tempmy.id]['chatlog']['group'+that.chatto]
							        }
								}else{
									console.log(2)
									if(typeof(res.data[tempmy.id]['chatlog']['friend'+that.chatto])=='undefined'){
										
									}else{
										list= res.data[tempmy.id]['chatlog']['friend'+that.chatto]
